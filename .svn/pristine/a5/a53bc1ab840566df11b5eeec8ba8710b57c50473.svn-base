...
var showToday = 1;
var imgDir = "/COMS/images/icon/";

var gotoString = "go today";
var todayString = "Today is ";
var weekString = "Week";
var scrollLeftMessage = "go to prev month"
var scrollRightMessage = "go to next month"
...
if(dom) {
    ...
    document.write ("<div onclick='bShow=true' id='calendar'    style='z-index:+999;position:absolute;visibility:hidden;'><table    width="+((showWeekNumber==1)?250:220)+" style='font-family:arial;font-size:11px;border-width:1;border-style:solid;border-color:#a0a0a0;font-family:arial; font-size:11px}' bgcolor='#ffffff'><tr bgcolor='#0000aa'><td><table width='"+((showWeekNumber==1)?248:218)+"'><tr><td style='padding:2px;font-family:arial; font-size:11px;'><font color='#ffffff'><B><span id='caption'></span></B></font></td><td align=right><a href='javascript:hideCalendar()'><IMG SRC='"+imgDir+"closeGray.gif' WIDTH='15' HEIGHT='13' BORDER='0' ALT='Close the Calendar'></a></td></tr></table></td></tr><tr><td style='padding:5px' bgcolor=#ffffff><span id='content'></span></td></tr>");
...
function closeCalendar() {
    ...
    if(_isGoneDayCheck){
		if(isGoneDay(yearSelected,monthSelected,dateSelected)){
			alert('오늘 이전 날짜를 선택할 수 없습니다.');
		}else ctlToPlaceValue.value = constructDate(dateSelected,monthSelected,yearSelected);
    }else ctlToPlaceValue.value = constructDate(dateSelected,monthSelected,yearSelected);
}

// 과거 체크
function isGoneDay(year,month,day){
    var selectedDate=new Date(year,month,day);
    var now = new Date();
    
//    alert(selectedDate.getTime());
//    alert(now.getTime()-86400);
    if(selectedDate.getTime() < now.getTime()-86400000) return true;
    else return false;
}
...
function popUpCalendar(ctl, ctl2, format,leftPoint,topPoint,isGoneDayCheck) {
    var leftpos = 0;
    var toppos = 0;
    
    if(isGoneDayCheck==true) _isGoneDayCheck=true;
    else _isGoneDayCheck=false;
    ...
    if(bPageLoaded) {
        if(crossobj.visibility == "hidden") {
            ...
            aFormat    = dateFormat.split(formatChar);
                ...
                crossobj.left =    leftPoint==-1 ? ctl.offsetLeft    + leftpos :    leftPoint;
                crossobj.top = topPoint==-1 ?    ctl.offsetTop +    toppos + ctl.offsetHeight +    2 :    topPoint;
                ...
                if (ctlNow!=ctl) {
                    popUpCalendar(ctl, ctl2, format,leftPoint,topPoint);
                }
            }
            ctlNow = ctl;
        }
